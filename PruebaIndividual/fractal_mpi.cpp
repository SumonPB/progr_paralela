...
uint32_t acotado_2(double x, double y)
{
    /*
    dados: c, z0
    Burn ship: z_{n+1} = (|Re(z_n)| + i|Im(z_n)|)^2 + c
    */
    ...
    {
        const double xr = std::abs(zr);
        const double yi = std::abs(zi);

        const double dr = xr * xr - yi * yi + c.real();
        const double di = 2.0 * xr * yi + c.imag();
        zr = dr;
        zi = di;
